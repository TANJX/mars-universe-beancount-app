...
export function extractFuture() {
  ...
  Array.from(transactions)
    ...
    .forEach((transaction) => {
      ...
      const descriptionRaw = descriptionElement
      ...
      const amountText = amountElement
        ? amountElement.innerText.trim()
        : "$0.00"
      ...
      if (
        descriptionRaw.includes("MTA*NYCT PAYGO") ||
        descriptionRaw.includes("OMNY")
      ) {
      ...
      result.push(
        `  Assets:Checking:Future                    ${amount.toFixed(2)} USD`
      )
